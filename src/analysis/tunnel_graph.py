...
class TunnelGraph(object):
	...
	def parse_tunnel_log(self):
		...
		self.converged_group_tputs = {k:(v/t/1000000.0) for k, v in group_totals.items()}
	...
	def run(self):
		...
		tunnel_results['converged_tput'] = self.converged_tput
		...
		for flow_id in self.flows:
			if flow_id != 0:
				...
				group_data[group_id]['tput'] = self.converged_group_tputs[group_id]
