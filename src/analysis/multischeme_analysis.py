...
def plot_multischeme_summary(data_dirs, output_dir):
	...
	for scheme_id, data_dir in enumerate(data_dirs):
		...
		for k, v in {'vs cubic':mixed, '': solo}.items():
			...
			loss = np.mean(v['loss'])
			throughput = np.mean(v['throughput'])
			capacity = np.mean(v['bottleneck_tput'])
			plt.figure(1)
			...
			plt.figure(3)
			plt.plot(throughput/capacity, 0, 'o' if k=='' else 'x', label='%s %s'%(scheme, k), color=colors[scheme_id%len(colors)])
