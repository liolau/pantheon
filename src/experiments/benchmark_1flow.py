...
class Benchmark():
	def __init__(self, scheme_a, runs, runtime, delay_range, ramdisk = True, tmp_dir='./tmp_data', data_dir = './data', verbose=False):
	...
	def build_experiments(self):
		...
		routers = 11
		delays = self.delays
		self.solo =  self.build_rtt_experiments(self.scheme_a, delays, runs, runtime, routers)
                self.mixed = self.build_rtt_experiments(self.scheme_a, delays, runs, runtime, routers)
	...
	def build_rtt_experiments(self, scheme_a, delays, runs, runtime, routers):
		...
		res = []
		for rtt_a in rtt_unfairness_routers:
			...
			flows = [{'scheme':scheme_a, 'sender_router':rtt_a, 'count':1, 'flow_info':{'name':'%s_%d'%(scheme_a, rtt_a.args['delay'])}}]
		        print(rtt_a.args['delay'])

			exs = [	Experiment(  '1x%s%dms_queue%dB'%(scheme_a, rtt_a.args['delay'], q_size),
				flows,
				router,
				runtime=runtime,
				interval=0,
	...
	def run(self):
		...
		ex_parameters = ['bottleneck_tput', 'q_size', 'scheme_a', 'rtprop_a', 'runtime']
...
if __name__ == '__main__':
	...
	b = Benchmark(args.scheme_a, args.runs, args.runtime, args.delay_range, ramdisk = not args.no_ramdisk, tmp_dir = args.tmp_dir, data_dir = args.data_dir, verbose = args.verbose)
	b.run()
