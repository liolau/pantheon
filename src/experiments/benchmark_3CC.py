...
class Benchmark():
	def __init__(self, scheme_a, scheme_b, scheme_c, runs, runtime, delay_range, ramdisk = True, tmp_dir='./tmp_data', data_dir = './data', verbose=False):
	...
	def build_experiments(self):
		...
		routers = 11
		delays = self.delays
		self.solo =  self.build_rtt_experiments(self.scheme_a, self.scheme_b, self.scheme_c, delays, runs, runtime, routers)
                self.mixed = self.build_rtt_experiments(self.scheme_a, self.scheme_b, self.scheme_c, delays, runs, runtime, routers)
	...
	def build_rtt_experiments(self, scheme_a, scheme_b, scheme_c, delays, runs, runtime, routers):
		...
		res = []
		for rtt_a, rtt_b, rtt_c in itertools.combinations_with_replacement(rtt_unfairness_routers, 3):
			...
			exs = [	Experiment(  '1x%s%dms_1x%s%dms_1x%s%dms_queue%dB'%(scheme_a, rtt_a.args['delay'],scheme_b, rtt_b.args['delay'], scheme_c, rtt_c.args['delay'], q_size),
				flows,
				router,
				runtime=runtime,
				interval=0,
	...
	def build_router_range(self, mbps, delay, num_routers, range_factor=10):
		...
		current_queue_size = int(bdp_bytes) / math.sqrt(nof_flows)
	...
	def run(self):
		ex_identifiers = ['ex_name', 'run_id']
                ex_parameters = ['bottleneck_tput', 'q_size', 'scheme_a', 'scheme_b', 'scheme_c', 'rtprop_a', 'rtprop_b', 'rtprop_c', 'runtime']
...
if __name__ == '__main__':
	...
	b = Benchmark(args.scheme_a, args.scheme_b, args.scheme_c, args.runs, args.runtime, args.delay_range, ramdisk = not args.no_ramdisk, tmp_dir = args.tmp_dir, data_dir = args.data_dir, verbose = args.verbose)
	b.run()
